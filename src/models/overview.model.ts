...
export const createOverview = async (overview: IOverview): Promise<IOverview> => {
  const { title, introduction, conclusion, researchFocus, imageGallery } = overview;
  const { rows } = await pool.query(
    `INSERT INTO overviews (title, introduction, conclusion, research_focus, image_gallery)
     VALUES ($1, $2, $3, $4, $5) RETURNING *`,
    [title, JSON.stringify(introduction), JSON.stringify(conclusion), JSON.stringify(researchFocus), JSON.stringify(imageGallery)]
  );
  return rows[0];
};
...
export const updateOverview = async (id: number, overview: IOverview): Promise<IOverview | null> => {
  ...
  const { rows } = await pool.query(
    ...
    [title, JSON.stringify(introduction), JSON.stringify(conclusion), JSON.stringify(researchFocus), JSON.stringify(imageGallery), id]
  );
  return rows.length ? rows[0] : null;
};
